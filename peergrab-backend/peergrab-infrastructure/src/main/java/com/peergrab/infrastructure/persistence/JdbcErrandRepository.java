...
public class JdbcErrandRepository implements ErrandRepository {
    ...
    @Override
    public List<Errand> findConfirmTimeoutAfter(long timeoutSeconds, Instant afterAt, long afterId, int limit) {
        if (afterAt == null) return findConfirmTimeout(timeoutSeconds, limit);
        Timestamp after = Timestamp.from(afterAt);
        return jdbc.query("""
                SELECT * FROM errand
                 WHERE status = 'LOCKED'
                   AND locked_at < DATE_SUB(NOW(3), INTERVAL ? SECOND)
     ...
     * 状态和版本 CAS 使其中一个失败；资金幂等还有 escrow 状态 CAS
     * 与 ledger 唯一索引两道保护。
     */
    @Override
    ...
    public int casSettleFromDispute(long errandId, long expectedVersion) {
        return jdbc.update("""
                UPDATE errand
                   SET status = 'SETTLED', version = version + 1
                 WHERE id = ? AND status = 'DISPUTED' AND version = ?
                """, errandId, expectedVersion);
    }
    ...
    @Override
    public List<Errand> findAutoSettleDueAfter(long autoSettleSeconds, Instant afterAt, long afterId, int limit) {
        if (afterAt == null) return findAutoSettleDue(autoSettleSeconds, limit);
        Timestamp after = Timestamp.from(afterAt);
        return jdbc.query("""
                SELECT * FROM errand
                 WHERE status = 'DELIVERED'
