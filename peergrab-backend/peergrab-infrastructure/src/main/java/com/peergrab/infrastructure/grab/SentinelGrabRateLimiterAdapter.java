package com.peergrab.infrastructure.grab;
...
@Primary
@Component
@ConditionalOnProperty(name = "peergrab.grab.limit.type", havingValue = "sentinel")
public class SentinelGrabRateLimiterAdapter implements GrabRateLimiterPort {
    ...
    public SentinelGrabRateLimiterAdapter(
            @Value("${peergrab.grab.limit.enabled:true}") boolean enabled,
            @Value("${peergrab.grab.limit.per-second:500}") int permitsPerSecond,
            @Value("${peergrab.grab.limit.resource:peer_grab_grab}") String resourceName) {
