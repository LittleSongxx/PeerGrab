...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT,
        properties = {"peergrab.auth.allow-header-identity=true"})
...
class ApiEndpointIT {
     ...
     */
    @Autowired com.peergrab.domain.errand.ports.ErrandCachePort errandCache;
    ...
    private String login(TestRestTemplate rest, long userId) {
        var resp = rest.postForEntity("/api/auth/login",
                Map.of("userId", userId, "password", userId == ARBITRATOR
                        ? "integration-arbitrator-password"
                        : userId == RUNNER ? "integration-runner-2001-password"
                        : "integration-publisher-password"), Map.class);
    ...
    private long available(String accessToken) {
        var wallet = data(rest.exchange("/api/wallet", HttpMethod.GET, bearer(accessToken), Map.class));
        return Long.parseLong(String.valueOf(wallet.get("availableCents")));
    }
    ...
    void full_lifecycle_over_http() {
        ...
        long publisherBefore = available(pubToken);
        long runnerBefore = available(runnerToken);
        ...
        // 钱包：发单人余额减少（托管转移走）
        assertEquals(publisherBefore - 2000, available(pubToken), "发布只托管本任务的悬赏");
        ...
        // 跑腿钱包到账
        assertEquals(runnerBefore + 1900, available(runnerToken), "跑腿余额应增加扣除 5% 佣金后的所得");
    ...
    void cleanupActionMatrix() {
        ...
        jdbc.update("DELETE FROM escrow_order WHERE errand_id BETWEEN 910000000000 AND 910000000006");
        jdbc.update("DELETE FROM errand WHERE id BETWEEN 910000000000 AND 910000000006");
        for (long id = 910_000_000_000L; id <= 910_000_000_006L; id++) {
            errandCache.evict(id);
        }
    }
    ...
    @Test
    @Order(6)
    @DisplayName("消息仅本人可标记已读，重复操作无副作用")
    void notification_mark_read_requires_recipient() {
        String publisher = login(rest, PUB);
        String runner = login(rest, RUNNER);
        long id = System.currentTimeMillis() * 1000 + java.util.concurrent.ThreadLocalRandom.current().nextInt(1000);
        jdbc.update("INSERT INTO notification (id, msg_key, user_id, errand_id, type, content) "
                        + "VALUES (?, ?, ?, ?, 'SETTLED', 'read test')",
                id, "read-test-" + id, PUB, errandId);
        try {
            rest.exchange("/api/notifications/" + id + "/read", HttpMethod.POST, bearer(runner), Map.class);
            assertEquals(0, jdbc.queryForObject("SELECT read_flag FROM notification WHERE id = ?", Integer.class, id));

            var marked = rest.exchange("/api/notifications/" + id + "/read", HttpMethod.POST,
                    bearer(publisher), Map.class);
            assertEquals("OK", marked.getBody().get("code"));
            rest.exchange("/api/notifications/" + id + "/read", HttpMethod.POST, bearer(publisher), Map.class);
            assertEquals(1, jdbc.queryForObject("SELECT read_flag FROM notification WHERE id = ?", Integer.class, id));
        } finally {
            jdbc.update("DELETE FROM notification WHERE id = ?", id);
        }
    }

    @Test
    @Order(7)
    @DisplayName("发布缺少 rewardCents 返回业务参数错误而非拆箱异常")
    void publish_without_reward_returns_invalid_argument() {
        String token = login(rest, PUB);
        var response = rest.exchange("/api/errands", HttpMethod.POST,
                bearerJson(token, Map.of("type", "DELIVERY", "title", "missing reward",
                        "slotTotal", 1)), Map.class);
