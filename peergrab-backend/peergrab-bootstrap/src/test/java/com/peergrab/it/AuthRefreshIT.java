...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT,
        properties = {"peergrab.auth.mode=jwt", "peergrab.mq.enabled=false", "peergrab.auth.refresh-ttl-minutes=120"})
...
class AuthRefreshIT {
    ...
    void refresh_token_rotates_and_old_token_is_invalidated() {
        ...
        // Logout revokes both the active access token and the remaining refresh token.
        var headers = new org.springframework.http.HttpHeaders();
        headers.setBearerAuth(nextAccessToken);
        var logout = rest.exchange("/api/auth/logout", org.springframework.http.HttpMethod.POST,
                new org.springframework.http.HttpEntity<>(Map.of("refreshToken", nextRefreshToken), headers), Map.class);
        assertEquals("OK", logout.getBody().get("code"));
        Map<?, ?> afterLogout = rest.postForEntity("/api/auth/refresh",
                Map.of("refreshToken", nextRefreshToken), Map.class).getBody();
        assertEquals("UNAUTHORIZED", afterLogout.get("code"));
    }
}
