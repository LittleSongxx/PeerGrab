...
@SpringBootTest(properties = {
        "peergrab.mq.enabled=false",
        "peergrab.settle.commission-rate=0.05"
})
...
class SettleConcurrencyIT {
    ...
    private static boolean scenarioAPassed;
    private static boolean scenarioBPassed;
    private static int recordedSettlements;
    private static long recordedTps;
    private static int recordedFailures;
    private static int recordedDeadlocks;
    ...
    static void finishRun() {
        if (run != null) {
            run.finish(scenarioAPassed && scenarioBPassed ? "PASS" : "FAIL", String.format(
                    "{\"scenario\":\"S4\",\"scenarioASettled\":%d,\"tps\":%d,"
                            + "\"clientFailures\":%d,\"deadlocks\":%d,\"scenarioBPassed\":%s}",
                    recordedSettlements, recordedTps, recordedFailures, recordedDeadlocks, scenarioBPassed));
            run.close();
        }
    }
    ...
    void concurrent_settle_distinct_errands() throws Exception {
        ...
        long tps = Math.round(actualSettled * 1000.0 / Math.max(elapsed, 1));
        ...
        assertEquals(COUNT, actualSettled, "每个任务都必须在数据库中恰好完成一次结算");
        ...
        recordedSettlements = actualSettled;
        recordedTps = tps;
        recordedFailures = failed.get();
        recordedDeadlocks = deadlock.get();
    }
