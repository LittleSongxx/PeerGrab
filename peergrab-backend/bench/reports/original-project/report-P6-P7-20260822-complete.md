...
> 来源类别：原版项目历史实验，非 PeerGrab 阿里云 ECS 实测。
...
```bash
docker exec -i peergrab-mysql mysql -uroot -ppeergrab123456 peer_grab < docker/migrate-p6.sql
```
...
- 新增 `PeerGrabShardingRuleFactory`，产出 ShardingSphere `ShardingRuleConfiguration`。
- 新增 `PeerGrabModuloShardingAlgorithm`，实现 ShardingSphere `StandardShardingAlgorithm`。
...
- `PeerGrabShardingRuleFactoryTest` 使用 ShardingSphere 内置 `ClassBasedShardingAlgorithm` 实例化并调用本项目算法，验证 `campus_id=7` 路由到 `peer_grab_3`。
...
```bash
java -cp "peergrab-bench/target/classes:$(cat /tmp/peergrab-bench.cp)" \
  com.peergrab.bench.RampLoadClient http://127.0.0.1:8080 50,100,200,400,800 60
```
...
- `peergrab.auth.allow-header-identity=true`
- `peergrab.grab.limit.type=sentinel`
- `peergrab.grab.limit.per-second=100`
...
```bash
java -cp "peergrab-bench/target/classes:$(cat /tmp/peergrab-bench.cp)" \
  com.peergrab.bench.SpikeLoadClient http://127.0.0.1:8080 1000 1
```
...
并额外设置 `peergrab.credit.max-ongoing=10000` 避免多轮压测复用固定 runnerId 触发在途单上限。
...
```bash
java -cp "peergrab-bench/target/classes:$(cat /tmp/peergrab-bench.cp)" \
  com.peergrab.bench.RampLoadClient http://127.0.0.1:8080 50,100,200,400,800 15
```
