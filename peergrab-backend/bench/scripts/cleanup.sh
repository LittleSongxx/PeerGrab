...
#
# 用法：
#   仅限 COMPOSE_PROJECT_NAME=peergrab-bench-* 的独立可销毁环境，
#   且须设置 PEERGRAB_BENCH_DISPOSABLE=YES；其他项目一律拒绝。
...
MYSQL_CONTAINER="${MYSQL_CONTAINER:-$("${COMPOSE[@]}" ps -q mysql)}"
REDIS_CONTAINER="${REDIS_CONTAINER:-$("${COMPOSE[@]}" ps -q redis)}"
if [ -z "$MYSQL_CONTAINER" ] || [ -z "$REDIS_CONTAINER" ]; then
  echo "MySQL/Redis Compose containers not found; set MYSQL_CONTAINER and REDIS_CONTAINER if using another stack" >&2
  exit 1
fi
DB="${DB:-peer_grab}"
# 从容器环境读取密码，适配 Compose 自定义 PEERGRAB_MYSQL_PASSWORD。
# 使用数组保留参数边界，避免数据库名和 SQL 被 shell 再次展开。
MYSQL=(docker exec -i "$MYSQL_CONTAINER" sh -c
  'db="$1"; shift; MYSQL_PWD="$MYSQL_ROOT_PASSWORD" exec mysql -uroot -D "$db" -N -B "$@"'
  sh "$DB")
...
# These values are interpolated into SQL below; accept only the script's own run-id
# format, a calendar date and decimal digits before composing any query.
if [[ ! "$KEEP_LAST" =~ ^[0-9]+$ ]]; then
  echo "--keep-last 只接受非负整数" >&2; exit 1
fi
if [ -n "$RUN_ID" ] && [[ ! "$RUN_ID" =~ ^[0-9]{8}-[0-9]{6}(-[0-9]+)?$ ]]; then
  echo "--run 只接受 yyyyMMdd-HHmmss[-序号] 格式" >&2; exit 1
fi
if [ -n "$BEFORE" ] && [[ ! "$BEFORE" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]]; then
  echo "--before 只接受 YYYY-MM-DD 格式" >&2; exit 1
fi

# This legacy script resets shared wallet balances after deleting rows. Refuse to run
# unless the operator explicitly targets a disposable benchmark Compose project.
if [ "${PEERGRAB_BENCH_DISPOSABLE:-}" != "YES" ]; then
  echo "拒绝清理：须在独立可销毁压测环境设置 PEERGRAB_BENCH_DISPOSABLE=YES" >&2
  exit 1
fi
for container in "$MYSQL_CONTAINER" "$REDIS_CONTAINER"; do
  ...
  case "$project" in
    peergrab-bench-*) ;;
    *) echo "拒绝清理：容器 $container 不属于 peergrab-bench-* 项目" >&2; exit 1 ;;
  esac
done
...
DELETE p FROM publish_request p   JOIN tmp_orphan t ON t.id = p.errand_id;
...
DELETE p FROM publish_request p   JOIN tmp_ids t ON t.id = p.errand_id;
