-- P6 增量迁移：给任务共片表补分片键。
-- 用法：
--   docker compose --env-file docker/.env -f docker/docker-compose.yaml exec -T mysql sh -c 'MYSQL_PWD="$MYSQL_ROOT_PASSWORD" mysql -uroot peer_grab' < docker/migrate-p6.sql
