...
> 项目代号：`peergrab`（校园跑腿抢单市场）
> 文档定位：记录方案选择与演进。历史候选方案和后续设想保留作讨论；当前运行事实以[仓库首页](../../README.md)及源码为准。
...
PeerGrab 聚焦单名额校园跑腿交易中的并发裁决和资金一致性。
...
1. **抢单是天然的并发考题。** 一个任务 N 人同时抢、只能成功一个，适合验证 Lua 原子判定、数据库 CAS 与幂等设计；超卖与重复抢都可由持久记录核验。
2. **钱相关的系统面试官天然爱追问。** 托管、结算、退款、对账，逼着你把事务边界、最终一致、幂等键设计讲清楚，糊不过去。
3. **超时流转把延迟队列做成了硬需求。** 不是为了用 MQ 而用 MQ，是业务上真的需要"5 分钟后如果还没确认就换人"。
4. **缓存一致性有真实场景。** 任务详情读多写少，MySQL 保存最终事实，Redis 提供缓存；任务列表走 MySQL 索引。ES/Canal 是已摘除的历史候选方案。
...
关键在那条虚线：`infrastructure` 依赖 `domain`，而不是反过来。领域层定义 `ports`（接口），基础设施层提供适配器实现；替换 Redis 或 JdbcTemplate 适配器不应改领域规则，测试领域逻辑也无需启动中间件。
...
peergrab-backend/
├── pom.xml                  # 聚合 pom
├── peergrab-shared/         # 金额、错误码、结果、雪花 ID
├── peergrab-domain/         # 任务、抢单、钱包、信用、通知等领域规则和端口
├── peergrab-application/    # 用例与事务边界
├── peergrab-infrastructure/ # JdbcTemplate、Redis、RocketMQ、Sentinel 适配器
├── peergrab-presentation/   # REST、WebSocket、鉴权与 DTO
├── peergrab-bootstrap/      # API 装配和集成测试
├── peergrab-worker/         # 延迟流转、补偿、校验和对账
├── peergrab-bench/          # 实验客户端
├── docker/                  # MySQL、Redis、RocketMQ 与全栈 Compose
├── bench/                   # 造数、校验、历史实验报告
└── docs/                    # 设计与演进记录
```
...
两个进程共享 `peergrab-application` 和 `peergrab-infrastructure`，所以业务逻辑不重复写，只是启动入口和装配不同。

### 4.4 分层规则用 ArchUnit 守住

架构文档写得再漂亮，几个月后也会烂掉——除非把规则写成测试。`peergrab-domain` 里放一组 ArchUnit 测试：

```java
@AnalyzeClasses(packages = "com.peergrab")
...
|---|---|---|---|
| 运行时 | JDK 21 | API、Worker 与压测客户端 | 线程与连接池配置 |
| 框架 | Spring Boot 3.5.8 | 依赖注入、事务、REST | 事务边界与传播 |
| 数据访问 | JdbcTemplate | 显式 CAS SQL 与受影响行数 | 索引与并发写入 |
| DB | MySQL 8（InnoDB，单库） | 任务/账户/流水/幂等表 | 行锁、事务与索引设计 |
| 分片实验 | ShardingSphere-JDBC 5.5.2 | 规则工厂与算法测试；运行环境仍是单 MySQL | 分片键与跨片事务取舍 |
| 缓存 | Redis 7 + Redisson | 抢单原子判定、任务详情缓存、候选 ZSET、布隆过滤器 | Lua 原子性、缓存重建、热 Key 治理 |
...
| 治理 | Sentinel | 抢单热点参数限流 | 限流阈值与业务拒绝 |
| 鉴权 | Redis Session / JWT 双实现 | 演示登录、refresh token 与身份拦截 | 会话与令牌权衡 |
| 实时 | WebSocket | 任务与资金事件推送；持久通知补读 | 断线重连与最终可见 |
| 状态机 | 领域层自研 enum + 事件表 | 任务状态合法流转 | 状态机 vs if-else、并发状态跃迁 |
| 调度 | Spring 定时任务 | 对账、消息重试、兜底扫描 | 幂等与积压恢复 |
| 观测计划 | 指标采集与看板尚未落地 | 下一轮需记录抢单结果、流转延迟、资金差异与资源使用 | 指标设计、链路追踪 |
...
**为什么使用 JdbcTemplate。** 抢单核心是 `UPDATE ... WHERE status=? AND version=?` 这种带条件 CAS，需要精确控制 SQL 和索引命中，并用 `affectedRows` 裁决冲突。JdbcTemplate 让仓储显式表达这些语义，领域模型仍与表结构解耦。
...
**为什么状态机自研而不用框架。** 状态机是领域层的核心资产，用 COLA StateMachine 这类框架会让 `peergrab-domain` 引入外部依赖，破坏"领域层零框架"的规则。自研的成本其实很低——一个枚举加一张流转表，加上 `canTransitTo()` 校验方法就够了，而且能顺手把"流转时必须带 version"这种并发约束写进领域模型里。
...
资金变更后需要通知信用分和 WebSocket 等后续处理。问题是：**本地事务提交了，但发消息失败了怎么办？**
...
| 雪崩 | 大批缓存同时到期，或 Redis 整体故障 | TTL 与随机抖动限制同刻过期；Redis 故障恢复行为仍需独立验证 |
...
**分布式 ID 用雪花算法。** 自增 ID 在分库分表下会冲突；UUID 无序会导致 InnoDB 页分裂。雪花 ID 趋势递增；当前实例的 `worker-id` 由配置指定，部署多个实例时必须分配不同值。

---

## 11 指标口径

下表仅归档 **2026-08 旧版同机实验**，不是现行版本容量或 SLA。接口名称、到达模型和故障条件不能跨场景换算；下一轮方法见[性能压测与架构决策方案](性能压测与架构决策方案.md)。

| 指标 | 口径定义 | 数据来源 | 历史记录 |
|---|---|---|---|
| 任务广场固定 GET 吞吐 | 旧版同机 60 秒闭环阶梯请求 | S2 阶梯加压报告 | 400 并发约 6455 QPS；800 并发约 6346 QPS，但有 146 个 IOException |
| 任务广场固定 GET P99 | 同上，仅成功样本口径 | S2 报告 | 400 并发 151ms；800 调参档 472ms |
| S1 瞬时抢单正确性 | 单任务 2000 请求竞争一个名额 | S1 尖峰报告 | 一轮零超卖；这不是并发上限证明 |
| 800 并发调参对比 | 同档配置变化 | P7 调参复测 | 错误数 146→0，QPS 6346→5205，P99 390ms→472ms |
...
**口径纪律**：引用数字时必须写明日期、代码版本、机器、负载模型和统计窗口。当前版本还缺乏完整独立环境稳态报告。
