...
public class ErrandActionResolver {
    ...
    public ErrandActionResolver(@Value("${peergrab.auth.arbitrator-id:9001}") long arbitratorId) {
    ...
    public Resolved resolve(Errand errand, long viewerId) {
        return resolve(errand.status(), errand.publisherId(), errand.grabberId(), viewerId);
    }

    /** Cached detail carries these fields, so action resolution needs no extra DB read. */
    public Resolved resolve(ErrandStatus status, long publisherId, Long grabberId, long viewerId) {
        List<String> actions = new ArrayList<>();

        boolean isPublisher = publisherId == viewerId;
        boolean isCurrentRunner = grabberId != null && grabberId == viewerId;
