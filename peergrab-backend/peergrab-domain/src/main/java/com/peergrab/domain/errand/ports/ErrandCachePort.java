...
public interface ErrandCachePort {
    ...
    /** 布隆过滤器的提示性判定。索引与数据库非同一事务，false 仍需回源核实。 */
    boolean mightExist(long errandId);

    /** 任务发布后登记到布隆 */
    boolean registerExisting(long errandId);

    /** 全量重建时先撤销就绪标记，防止把部分填充的索引当作完整索引。 */
    boolean beginExistenceIndexRebuild();

    /** 全部存量 id 成功登记后才设置就绪标记。 */
    boolean completeExistenceIndexRebuild();

    boolean existenceIndexReady();
