...
public interface GrabSlotPort {
    ...
    /** 任务从 LOCKED 回退后丢弃旧轮次的 Redis 名额快照；后续由数据库 CAS 兜底。 */
    void invalidate(long errandId);

    /** 是否有短期 Redis 预占位尚未完成 DB 落库，用于区分并发中与陈旧满额快照。 */
    boolean reservationPending(long errandId);
